...
```
编排者-工作者              流水线                群聊/辩论              层级
(Orchestrator-Worker)    (Pipeline)            (Group Chat)         (Hierarchical)
...
```
       ...
       派活   │   │   │   收结果
        ...
        ▼         ▼         ▼
   ┌────────┐┌────────┐┌────────┐
   │研究员    ││写作者   ││审校     │   ← 工作者: 职责单一、上下文独立、工具精简
...
```
原始输入 → [清洗 Agent] → [抽取 Agent] → [润色 Agent] → 最终输出
            提取正文       抽取要点        改写成文章
```
...
```
            ...
            ▼                   ▼
      ┌──────────┐        ┌──────────┐
      │ 前端组组长 │        │ 后端组组长 │   ← 中层: 既是上层的工作者, 又是下层的编排者
      └─────┬────┘        └─────┬────┘
        ┌───┴───┐          ┌────┴───┐
        ▼       ▼          ▼        ▼
      样式 W   交互 W     接口 W    数据 W
...
```
  用户: "写一篇关于'前端工程师为什么适合转型 Agent 开发'的短文"
                          │
                          ▼
                  ┌──────────────┐
                  │  Supervisor   │  ① 规划: 研究 → 写作 → 审校
                  └──────┬───────┘
        ┌────────────────┼────────────────┐
        ▼ ②派活           ▼ ③拿结果再派     ▼ ④拿结果再派
   ┌─────────┐      ┌─────────┐      ┌─────────┐
...
```
串行(有依赖):  研究 ──▶ 写作 ──▶ 审校        总耗时 = T研 + T写 + T审

并行(无依赖):  ┌─ 调研竞品A ─┐
              ├─ 调研竞品B ─┤ ──▶ 汇总        总耗时 ≈ max(TA, TB, TC) + T汇
              └─ 调研竞品C ─┘                （三个一起跑，省掉等待）
```
