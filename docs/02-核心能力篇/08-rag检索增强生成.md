...
```mermaid
flowchart LR
    A["小猫<br/>[0.21, 0.85, ...]"] --- B["猫咪<br/>[0.23, 0.83, ...]"]
    A --- C["汽车引擎<br/>[-0.7, 0.10, ...]"]
    A -.->|"向量很接近(意思相近)"| B
    A -.->|"向量离得远(意思无关)"| C
...
flowchart TD
    SRC["原文：...... 句A | 句B 句C | 句D ......"]
    SRC --> N["无重叠切块：[句A] [句B句C] [句D]<br/>(边界信息可能割裂)"]
    SRC --> O["有重叠切块：[句A 句B] [句B 句C 句D] ...<br/>(句B 在两块都出现，边界更平滑)"]
```
...
```mermaid
flowchart LR
    A["向量检索(快, 粗)"] --> B["候选 top-20"] --> C["重排模型(慢, 精)"] --> D["最终 top-5"] --> E["喂给模型"]
```
