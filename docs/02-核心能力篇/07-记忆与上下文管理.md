...
```
第 1 轮请求：[user: "我叫 Alice"]
              ← assistant: "你好 Alice"

第 2 轮请求：[user: "我叫 Alice",
              assistant: "你好 Alice",
              user: "我叫什么？"]        ← 把前两条也一起发
              ← assistant: "你叫 Alice"
```
...
```
历史: [m1, m2, m3, m4, m5, m6]   N=4
保留:         [m3, m4, m5, m6]   ← m1, m2 被丢弃
```
...
```
压缩前: [system][m1][m2]...[m40][m41][m42]   ← 42 条，快撑爆了
                └──── 旧的 1~40 ────┘
                       │ 交给模型总结
                       ▼
压缩后: [system][摘要: "用户在搭一个 Python 爬虫，已实现
                 基础抓取，讨论了 JS 渲染页面的处理方案……"]
                [m41][m42]              ← 只剩 摘要 + 最近 2 条
```
