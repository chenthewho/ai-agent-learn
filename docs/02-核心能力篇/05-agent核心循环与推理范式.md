...
```
用户提问 → 模型(要调工具) → 你执行工具 → 把结果给模型 → 模型(给答案) → 结束
```
...
```
  ...
  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌──────────────┐
  │  观察    │ →  │  思考    │ →  │  行动    │ →  │  得到工具结果  │
  │(messages)│    │(调用模型) │    │(执行工具) │    │(tool_result) │
  └─────────┘    └────┬────┘    └─────────┘    └──────────────┘
                      │                                │
                      │  模型不再调工具                 │
...
│  Planner（规划器）                                       │
│  输入：用户任务                                          │
│  输出：步骤清单 [1. 查坐标  2. 查天气  3. 给穿衣建议]      │
└───────────────────────┬──────────────────────────────┘
                        │
                        ▼
┌──────────────────────────────────────────────────────┐
│  Executor（执行器，内部往往是个 ReAct 小循环）            │
│  for 每个步骤:                                          │
│     执行这一步（可能调工具）                              │
│     若结果偏离预期 → 触发 Re-plan（让 Planner 改计划）    │
└──────────────────────────────────────────────────────┘
```
