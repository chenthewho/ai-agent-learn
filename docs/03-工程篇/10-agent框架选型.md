...
```mermaid
flowchart TD
    Start["你是前端/全栈，要做一个 Agent 应用"]
    Start --> Q1{"核心是 RAG / 海量异构数据源？"}
    Q1 -->|"是"| A1["LlamaIndex（数据层）<br/>或 AI SDK + 自建检索（轻量）"]
    Q1 -->|"否"| Q2{"需要复杂可控编排 /<br/>要对接 Python 算法生态？"}
    Q2 -->|"是"| A2["LangGraph（图状态编排）"]
    Q2 -->|"否"| Q3{"想纯 TS 搭完整 Agent 后端<br/>（Agent+工作流+记忆+RAG）？"}
    Q3 -->|"是"| A3["Mastra"]
    Q3 -->|"否"| Q4{"主要是'调模型 + 工具 + 流式 UI'（最常见）？"}
    Q4 -->|"是"| A4["Vercel AI SDK ← 前端默认起点"]
    Q4 -->|"否"| Q5{"明确要多 Agent 协作？"}
    Q5 -->|"是"| A5["CrewAI / AutoGen / OpenAI Agents SDK<br/>（且先确认真需要多 Agent）"]
```
