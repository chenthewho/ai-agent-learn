...
用户点了"发送"
   ...
   │     从请求发出到模型吐出第一个字。模型要先"读完"你的输入。
   │     输入越长、模型越大，TTFT 越久。
   │
   ├─ ② 生成速度 (tokens/秒)
   │     模型一个字一个字往外蹦的速度。输出越长，这段越久。
   │
   ├─ ③ 工具往返 (tool round-trip)
   │     模型说"调工具"→ 你执行（可能是网络请求/查库）→ 结果发回去。
   │     一次工具调用 = 一来一回，慢工具直接拖累整体。
   │
   └─ ④ 串行步数 (sequential steps)
         Agent 循环每多一步,就多一轮 ①+②+③。10 步串行 = 10 倍的来回。
```
...
```
请求 = [   tools   ][   system   ][        messages        ]
         └──────── 固定前缀 ────────┘└──── 每轮变化的尾巴 ────┘
                    ↑
              在这里放一个缓存断点(cache_control)
              这之前的内容被缓存；命中后这段只收 ~10% 的钱
```
