...
没有 MCP：M 个 Agent × N 个工具源 = M×N 套适配，每条线都是一次重写。
...
有 MCP：M 个客户端 + N 个 Server = M+N，每个工具源各实现一次 MCP Server。

```mermaid
flowchart TD
    AgentA["AgentA（MCP Client）"]
    AgentB["AgentB（MCP Client）"]
    AgentC["AgentC（MCP Client）"]
    MCP["MCP 统一协议"]
    GitHub["GitHub"]
    FS["文件系统"]
    DB["数据库"]
...
```mermaid
flowchart TD
    subgraph Host["Host 宿主应用（Claude 桌面端 / 某 IDE / 你自己写的 Agent 应用）——运行 LLM、管理对话、决定连哪些 Server"]
        C1["MCP Client"]
        C2["MCP Client"]
        C3["MCP Client"]
    end
    S1["MCP Server：文件系统<br/>暴露 Tools / Resources / Prompts"]
    S2["MCP Server：GitHub<br/>暴露 Tools / Resources / Prompts"]
    S3["MCP Server：你的业务库<br/>暴露 Tools / Resources / Prompts"]
    C1 -->|"1:1（stdio 或 HTTP）"| S1
    C2 -->|"1:1（stdio 或 HTTP）"| S2
    C3 -->|"1:1（stdio 或 HTTP）"| S3
```
...
```mermaid
flowchart BT
    Dispatch["分发层：MCP（标准化工具从哪来、长什么样）← 本章<br/>MCP Server 暴露工具 → 客户端列出 → 转成工具定义"]
    Model["模型层：Function Calling（工具调用）← 第 4 章，底层机制<br/>模型返回 tool_use → 执行 → tool_result 回填"]
    Dispatch -->|"MCP 工具最终也变成这种 tool_use / tool_result"| Model
```
...
flowchart TD
    Agent["你的 Agent"]
    Builtin["内置工具（服务端）<br/>厂商提供、服务端执行<br/>（网页搜索、代码执行...）"]
    Custom["自定义工具（你自己写）<br/>第 6 章那套、你的业务专属工具<br/>（查你的订单库、发你的业务通知...）"]
    McpTool["MCP 工具（标准接入）<br/>11.4 连现成 Server、11.5 自己写 Server<br/>（文件系统、GitHub、社区生态...）"]
    Agent --> Builtin
    Agent --> Custom
    Agent --> McpTool
```
