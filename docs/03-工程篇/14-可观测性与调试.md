...
```
Trace: "帮我查天气并推荐穿搭"  (总耗时 4.2s, 总成本 $0.013)
│
├─ Span: LLM call #1 (决策)            [0.0s → 1.1s]  输入 1.2k tok / 输出 80 tok
│        └─ 决定调用 get_weather(city="北京")
│
├─ Span: Tool call: get_weather       [1.1s → 1.4s]  入参 {city:"北京"} → "晴, 8℃"
│
├─ Span: LLM call #2 (基于天气推理)    [1.4s → 3.0s]  输入 1.4k tok / 输出 200 tok
│        └─ 决定调用 search_clothing(keyword="初春 8度")
│
├─ Span: Tool call: search_clothing   [3.0s → 3.5s]  入参 {...} → [3 条结果]
│
└─ Span: LLM call #3 (生成最终回复)    [3.5s → 4.2s]  输入 1.8k tok / 输出 150 tok
         └─ 输出最终穿搭建议
```
