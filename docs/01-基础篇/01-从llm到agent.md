...
```
                      ...
                      └──┬────────┬────────┬────────┬┘
                         │        │        │        │
              ┌──────────▼──┐  ┌──▼────┐ ┌─▼─────┐ ┌▼────────┐
              │ 规划 Planning │  │记忆    │ │工具    │ │行动      │
              │ 拆解任务、     │  │Memory │ │Tools  │ │Action   │
              │ 决定顺序、     │  │短期:   │ │查库、   │ │实际执行  │
              │ 反思纠错       │  │对话历史│ │调API、 │ │工具调用  │
              │              │  │长期:   │ │执行代码│ │并拿回结果│
              │              │  │向量库等│ │……     │ │          │
...
你的需求是什么?
│
├─ 一次输入 → 一次输出就能搞定?(翻译/摘要/分类/抽取)
│     └─ ✅ 用【单次调用】。最便宜最快,别想多。
│
├─ 需要和用户多轮对话,但本质还是"问答",不需要它去操作外部系统?
│     └─ ✅ 用【聊天机器人】(可能配 RAG 检索知识库)。
│
├─ 是个多步骤任务,但步骤和顺序你心里很清楚、基本固定?
│     └─ ✅ 用【工作流 Workflow】。你写代码编排,把模型当成
│        流程里的一两个"智能环节"。可控、可测、可预期。
│
└─ 任务是开放式的、步骤无法提前写死、需要模型根据中间结果
   临场决定下一步,而且这件事值得花更多成本和时间?
        └─ ✅ 这才轮到【Agent】。
```
...
```
纯 LLM 调用          只会"说",一问一答,困在纯文本盒子里
   │
   ▼
Chain(链式)         把多次调用串起来:第一步的输出当第二步的输入。
                     顺序是人写死的 → 本质是"工作流"。
   │
   ▼
ReAct(推理+行动)    关键转折:让模型在"推理(Reasoning)"和
                     "行动(Acting,调工具)"之间交替——
                     想一步、做一步、看结果、再想……
                     这是现代 Agent 循环的雏形。
   │
   ▼
自主 Agent           模型自己规划、调工具、反思纠错、长程执行,
                     人只给目标和护栏。今天的 Agent 大多是这一类。
```
...
```
【第一篇·基础篇】打地基
├─ 第 1 章 从 LLM 到 Agent          ← 你在这里(总纲)
├─ 第 2 章 大语言模型基础            ← token / 上下文 / 无状态 / 调 API
└─ 第 3 章 提示工程                  ← 怎么"说话"模型才听得懂、做得对
        (参见 ./02-大语言模型基础.md、./03-提示工程.md)

【第二篇·核心能力篇】Agent 的四块肉
├─ 工具调用(Tool / Function Calling) ← 给模型装手脚
├─ Agent 核心循环与推理范式          ← ReAct 等,把循环讲透
├─ 记忆与上下文管理                  ← 短期/长期记忆
├─ 检索增强生成(RAG)               ← 让模型查"外部知识库"
└─ 多 Agent 协作                    ← 多个 Agent 分工合作
        (参见 ../02-核心能力篇/)

【第三篇·工程篇】让它能上线、不出事
├─ 流式输出与前端集成、错误处理与重试、护栏与安全
├─ 可观测性(日志/追踪/评测)、成本与缓存优化
└─ 部署与扩展
        (参见 ../03-工程篇/,如流式输出见 ../03-工程篇/12-流式输出与前端集成.md)

【第四篇·实战篇】从零手写完整项目
└─ 不依赖框架,亲手把这个循环骨架做成能跑、能上线的真实 Agent
        (参见 ../04-实战篇/)
